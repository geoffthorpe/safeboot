...
# There are production environments running Debian versions that don't honor
# the init.defaultBranch settings, and always use "master" instead. If a later
# version changes its default, at least that will honor this setting, so in the
# interests of reliability we go with "master".
