...
#SERVER=hcp-attestsvc-hcp   # <-- passed in from "docker run" cmd-line
...
# Bypass some stuff unless explicitly enabled. (This will only work for local
# work - when the swtpm is a side-car container on the same host and docker
# daemon.)
